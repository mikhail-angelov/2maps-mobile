...
import {
  getLocalMapListAction,
  setPrimaryMapAction,
  setSecondaryMapAction,
  loadMapListAction,
  downloadMapAction,
  downloadMapByQRAction,
  removeLocalMapAction,
  cancelDownloadMapAction,
  getStorageMemoryInfo,
  importMapAction,
  moveMapToSdCardAction,
  moveMapToPhoneStorageAction,
  isFileValid,
  cancelTransferMapAction,
...
import {
  ...
  selectMapIsLoading,
  onLineMapList,
  selectAvailableMapList,
  selectMapError,
  selectDownloadProgress,
  selectMapIsDownLoading,
  selectMapIsRelocating,
  selectRelocateProgress,
} from '../reducers/map';
...
const removeStorageFromNamePattern = new RegExp(
  `:(${internalStorage}|${sdCardStorage})`,
);
        ...
        <View style={styles.helpArticle}>
          ...
          <View style={styles.helpArticleTextContainer}>
            <Text style={[styles.helpText]}>
              {t('firstArticleHelp.firstLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('firstArticleHelp.secondLine')}
              <Text
                style={styles.link}
                onPress={() => Linking.openURL(LINKING_URL)}>
                &nbsp;{t('EtoMesto.ru')}
              </Text>{' '}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('firstArticleHelp.thirdLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('firstArticleHelp.fourthLine')}
            </Text>
          </View>
        </View>
        <View style={styles.helpArticle}>
          ...
          <View style={styles.helpArticleTextContainer}>
            <Text style={styles.helpText}>
              {t('secondArticleHelp.firstLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('secondArticleHelp.secondLine')}
              <Text
                style={styles.link}
                onPress={() => Linking.openURL(LINKING_URL)}>
                &nbsp;{t('EtoMesto.ru')}
            ...
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('secondArticleHelp.thirdLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('secondArticleHelp.fourthLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('secondArticleHelp.firthLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('secondArticleHelp.sixthLine')}
            </Text>
          </View>
        </View>
        <View style={styles.helpArticle}>
          ...
          <View style={styles.helpArticleTextContainer}>
            <Text style={styles.helpText}>
              {t('thirdArticleHelp.firstLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('thirdArticleHelp.secondLine')}
            </Text>
            <Text style={[styles.helpText, styles.helpTextAddGap]}>
              {t('thirdArticleHelp.thirdLine')}
            </Text>
          </View>
        </View>
      </ScrollView>
...
const MapSettings: FC<Props> = ({
  ...
  useEffect(() => {
    requestWriteFilePermissions()
      .then(g => {
        console.log('no permissions', g);
        return getStorageMemoryInfo();
      })
      .then(response => {
  ...
  const allMaps: MapItem[] = [
    ...validList.map(({name, url, size, storage}: MapInfo) => ({
      id: name,
      name: `${name.replace(removeStorageFromNamePattern, '')} (${(
        size / 1000000
      ).toFixed(3)}M)`,
      file: url,
      loaded: true,
    ...
    ...availableMapList
      .filter(
        ({name}) =>
          !validList.find(
            item =>
              item.name.replace(removeStorageFromNamePattern, '') === name,
          ),
      )
      .map(({id, name, url, size}) => {
        return {
          id,
          name: `${name} (${(size / 1000000).toFixed(3)}M)`,
          file: url,
          loaded: false,
        };
      }),
  ...
  const confirmMovementMapToSdCard = (item: MapItem) => {
    if (isSDCardExist) {
      showModal({
        title: '',
        text: t('Move to SD Card', {name: item.name}),
        actions: [
      ...
      showModal({
        title: '',
        text: t('No SD Card', {name: item.name}),
  ...
  const confirmMovementMapToPhoneStorage = (item: MapItem) => {
    showModal({
      title: '',
      text: t('Move to Phone', {name: item.name}),
      actions: [
        {text: t('No'), type: ModalActionType.cancel},
        {
          text: t('Yes'),
  ...
  const confirmRemoving = (item: MapItem) => {
    showModal({
      title: t('Warning!'),
      text: t('Are you sure to remove the map?', {name: item.name}),
      actions: [
        {text: t('No'), type: ModalActionType.cancel},
        {
          text: t('Yes'),
  ...
  const processCapturedQR = (link: string) => {
    ...
    if (isFileValid(fileName)) {
      ...
      showModal({
        title: t('File name is not valid:', {name: fileName}),
        text: t('You have to try with ".sqlitedb" files'),
        actions: [{text: t('Ok'), type: ModalActionType.cancel}],
      });
    }
  };
      ...
      <View style={styles.row}>
        ...
        {item.loaded && (
          <View style={styles.listButtonsContainer}>
            {item.storage === internalStorage && (
              <Icon.Button
                style={styles.listButton}
                iconStyle={styles.listIconStorage}
                size={22}
                backgroundColor="transparent"
                name="phone-android"
                accessibilityLabel="move to sd card"
                onPress={() => confirmMovementMapToSdCard(item)}
              />
            )}
            {item.storage === sdCardStorage && (
              <Icon.Button
                style={styles.listButton}
                iconStyle={styles.listIconStorage}
                size={22}
                backgroundColor="transparent"
                name="sd-card"
                accessibilityLabel="move to mobile"
                onPress={() => confirmMovementMapToPhoneStorage(item)}
              />
            )}
            <Icon.Button
              style={styles.listButton}
              iconStyle={styles.listIconRemove}
              size={22}
              backgroundColor="transparent"
              name="delete-outline"
              accessibilityLabel="remove"
              onPress={() => confirmRemoving(item)}
            />
          </View>
        )}
        {!item.loaded && isAuthenticated && (
          <Button
            titleStyle={{color: purple}}
            type="clear"
            onPress={() => downloadMap({id: item.id, name: item.file})}
            title="download"
          />
        )}
      </View>
    ...
    <Modal style={styles.container} visible onRequestClose={close}>
      ...
      {isRelocating && (
        <View style={styles.loadingOverlay}>
          ...
          <View style={styles.progressBar}>
            <ProgressBar progress={progressForRelocate} />
          </View>
        ...
        <View>
          ...
          <View style={{borderWidth: 1}}>
            <Picker
              ...
              mode="dropdown">
              {primaryList.map(({name}: PrimaryMapInfo) => (
                <Picker.Item
                  key={name}
                  label={name}
                  value={name}
          ...
          {isMemoryAvailable ? (
            <View style={{flexDirection: 'row'}}>
              <Text>{`${t(
                'Memory on phone',
              )} ${totalInternalMemory}/${availableInternalMemory} ${t(
                'free',
              )}, ${
                isSDCardExist
                  ? `${t(
                      'SD card',
                    )} ${totalExternalMemory}/${availableExternalMemory} ${t(
                      'free',
                    )}`
                  : 'no SD card'
              }`}</Text>
          ...
          <View style={styles.row}>
            <Text style={styles.notificationText}>
              {t('Supported map format .SQLiteDB')}
            </Text>
            <Text
              style={styles.link}
              onPress={() => Linking.openURL(ETO_MESTO_URL)}>
              EtoMesto.ru
            </Text>
          </View>
        </View>
        <View style={styles.availableMaps}>
          ...
          {!isAuthenticated && (
            <Button
              ...
              title={t('Login to download maps') as string}
              onPress={showAuth}
            />
          )}
        </View>
      ...
      {showQRReader && (
        <Modal>
          <QR
            close={() => setShowQRReader(false)}
            select={link => {
              processCapturedQR(link);
            }}
          />
        </Modal>
      )}
      {showHelp && (
        <MapModal onRequestClose={() => setShowHelp(false)}>
          <Help />
        </MapModal>
      )}
    </Modal>
